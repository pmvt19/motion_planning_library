...
from motion_planning.database import ClusteredDatabase, Database
...
class TestClusteredDatabase(unittest.TestCase):
    ...
    def create_database(self) -> Database:
        ...
        populate_db(db, mp_sampler, num_envs=2, num_tasks_per_env=2, smooth_paths=False)
        return db
    ...
    def test_merge_databases_with_clusters(self):
        ...
        num_clustered_paths = sum(
            [len(cluster) for cluster in self.db.clusters.values()]
        )
    ...
    def test_clustered_database_subsampling(self):
        ...
        subsampled_db = self.db.subsample_database(
            num_paths_per_cluster=num_paths_per_cluster
        )
