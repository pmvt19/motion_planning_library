...
class InformedRRTStar(RRTStar):
    def __init__(self, env: RobotSpace, delta: float = 0.5, rewire_radius: float = 1, max_rewire_neighbors: int = 20):
        super().__init__(env=env, delta=delta, rewire_radius=rewire_radius, max_rewire_neighbors=max_rewire_neighbors)
    ...
    def sample_point_in_ellipse(self, path_length: float) -> NumpyState:
        ...
        while self.env.dist(sampled_point, self.start) + self.env.dist(sampled_point, self.target) > path_length:
            sampled_point = self.env.sample_valid_point()
