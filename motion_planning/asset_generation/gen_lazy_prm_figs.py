...
def lazy_search(
    ...
    while i < max_iter:
        ...
        path_edge_validities = prm.env.batch_is_valid_edge(
            start_states=start_edge_states, end_states=end_edge_states
        )
        ...
        invalid_edges = [
            (start_edge_states[idx], end_edge_states[idx]) for idx in invalid_edge_idxes
        ]
...
def search(prm: LazyPRM, start, target, save_figs=False):
    prm.start = start
    prm.target = target
