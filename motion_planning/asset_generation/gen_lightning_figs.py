...
from motion_planning.search import RRT, Lightning
...
def compute_candidate_paths(lightning: Lightning, start, target, n=5):
    ...
    all_path_states = np.array(
        [state.value for path in lightning.db.paths for state in path]
    )
...
def draw_candidate_paths(save_fig=False):
    ...
    best_n_path_idxes, path_idxes, all_path_states = compute_candidate_paths(
...
def draw_selected_path(save_fig=False):
    ...
    plt.plot(
        invalid_path_states[:, 0],
        invalid_path_states[:, 1],
        marker="o",
        color="#C00A0A",
        zorder=2,
...
def repair_path(lightning: Lightning, path, path_validities):
    path = path.path
    ...
    for seg_start_idx, seg_end_idx in intervals:
        repaired_segment = rrt.search(
            path[seg_start_idx - 1],
            path[seg_end_idx],
            max_steps=lightning.max_repair_steps,
        )
    ...
    for i in reversed(range(len(intervals))):
        ...
        path = (
            path[0:seg_start_idx] + repaired_segments[i].path + path[seg_end_idx + 1 :]
        )

    start_to_path = rrt.search(
        lightning.start, path[0], max_steps=lightning.max_repair_steps
    )
    path_to_target = rrt.search(
        path[-1], lightning.target, max_steps=lightning.max_repair_steps
    )

    return (
        start_to_path.path + path + path_to_target.path,
        repaired_segments,
        start_to_path,
        path_to_target,
    )
...
def draw_path_repair(save_fig=False):
    ...
    final_path, repaired_segments, start_to_path, path_to_target = repair_path(
        lightning, path, path_validity
    )
    env.space.draw_environment(plt.gca())
    plt.scatter(*start.value, color="green", s=100, zorder=2)
    plt.scatter(*target.value, color="red", s=100, zorder=2)
    ...
    for repaired_segment in repaired_segments:
        ...
        plt.plot(
            numpy_path[:, 0], numpy_path[:, 1], marker="o", label="Repaired Segments"
        )

    # Plot Connection to Task Segments
    numpy_path = np.array([state.value for state in start_to_path])
    plt.plot(
        numpy_path[:, 0], numpy_path[:, 1], marker="o", label="Start to Path Segment"
    )

    numpy_path = np.array([state.value for state in path_to_target])
    plt.plot(
        numpy_path[:, 0], numpy_path[:, 1], marker="o", label="Path to Target Segment"
    )
...
def draw_final_path(save_fig=False):
    ...
    numpy_path = np.array([state.value for state in final_path])
    plt.plot(
        numpy_path[:, 0], numpy_path[:, 1], marker="o", zorder=0, label="Final Path"
    )
