...
from motion_planning.search import RRT, RRTStar
from motion_planning.space import PointRobot
from motion_planning.utils import set_numpy_seed

# IDEA:
# Make two side-by-side graphs, one with RRT and the other with RRT*
# RRT will stop at the first path found while RRT* will continue 
# searching beyond the initial path to improve the path
...
def rrt_and_rrt_star_compare(save_figs=False):
    ...
    start, target = (
        env.make_state(np.array([0.0, 0.0])),
    ...
    if save_figs:
        plt.savefig("saves/rrt_star/steps/final.png")
    else:
        plt.show()
...
def rrt_star_animation():
    ...
    path = rrt.search(
        start, target, max_steps=1000, goal_bias=0.0, animate_search_tree=True
    )
