...
class DubinsCar(NonHolonomicRobot):
    ...
    def generate_costmetic_robot_representation(self, state):
        ...
        fr_wheel = create_rectangle_geometry(
            x_loc=x + self.car_width / 2 + self.x_offset,
            y_loc=y + self.car_length / 2 - self.y_offset,
            x_width=self.wheel_width,
            y_length=self.wheel_length,
        )

        fl_wheel = create_rectangle_geometry(
            x_loc=x - self.car_width / 2 - self.x_offset,
            y_loc=y + self.car_length / 2 - self.y_offset,
            x_width=self.wheel_width,
            y_length=self.wheel_length,
        ...
        fr_wheel = affinity.rotate(
            fr_wheel, theta, use_radians=True, origin=robot_centroid
        )
        fl_wheel = affinity.rotate(
            fl_wheel, theta, use_radians=True, origin=robot_centroid
        )
        ...
        velocity_arrow_stem = affinity.rotate(
            velocity_arrow_stem, theta, use_radians=True, origin=robot_centroid
        )
