...
class PlanarMobileArm(HolonomicRobot):
    def __init__(
        ...
        if arm_lengths is not None:
            assert num_links == len(arm_lengths), (
                "Num Links must be equal to the list size of arm lengths"
            )
    ...
    def generate_robot_representation(self, state):
        ...
        robot = create_rectangle_geometry(
            x_loc=x, y_loc=y, x_width=self.base_width, y_length=self.base_length
        )
        ...
        end_effector_point_pairs = self.create_end_effector_representation(
            joint_positions[-1]
        )
        ...
        for i in range(len(end_effector_point_pairs)):
            ...
            ee_link = affinity.rotate(
                ee_link,
                angle=(np.sum(thetas) + rotation_offset),
                use_radians=True,
                origin=list(joint_positions[-1]),
            )
            ee.append(ee_link)
    ...
    def inverse_kinematics(self, target_ee_position):
        ...
        for i in range(num_steps):
            ...
            dist_mat = pairwise_distances(
                target_ee_position.reshape(1, -1), ee_positions
            )
    ...
    def batch_sample_point(self, num_points) -> np.ndarray:
        issue_warning(
            True, "Batch Sample Points is hardcoded for planar mobile arm", "warning"
        )
        return np.random.uniform(
            low=np.array([-10, -10, 0, 0, 0]),
            high=np.array([10, 10, 2 * np.pi, 2 * np.pi, 2 * np.pi]),
            size=(num_points, 5),
        )
    ...
    def batch_forward_kinematics(self, states: np.ndarray) -> np.ndarray:
        ...
        arm_bases = np.vstack(
            (states[:, 0], states[:, 1] + self.base_length / 2)
        ).T  # (B, 2)
        ...
        point_der = (
            self.arm_lengths.reshape(1, self.num_links, 1) * normalized_link_points
        )  # (B, num_links, 2)
        joint_pos_untranslated = np.cumsum(point_der, axis=1)  # (B, num_links, 2)
        joint_pos = joint_pos_untranslated + arm_bases.reshape(-1, 1, 2)
        return np.concatenate((arm_bases.reshape(-1, 1, 2), joint_pos), axis=1)

    def batch_create_end_effector_segments(
        self, base_points: np.ndarray, thetas: np.ndarray
    ) -> np.ndarray:
        """
        ...
        batch_ee_points = np.concatenate(
            (ee_arm_1, ee_arm_2, ee_arm_3, ee_arm_4), axis=0
        )
        ...
        rotation_mats = np.array(
            [
                [cos, -sin, -xs * cos + ys * sin + xs],
                [sin, cos, -xs * sin - ys * cos + ys],
                [np.zeros_like(cos), np.zeros_like(cos), np.ones_like(cos)],
            ]
        )

        batch_ee_points_homogeneous = np.concatenate(
            (batch_ee_points, np.ones(shape=(len(base_points) * 4, 2, 1))), axis=2
        ).reshape(-1, 4, 2, 3)
    ...
    def batch_get_robot_representations(self, states: np.ndarray) -> dict:
        rectangles = np.stack(
            (
                states[:, 0],
                states[:, 1],
                np.ones((states.shape[0])) * self.base_width,
                np.ones((states.shape[0])) * self.base_length,
            ),
            axis=1,
        )
        ...
        This is due to the fact that the segments of the robot arms and the
        ee lines are not the same lengths. ApproximationSpace is better optimized
        for lines of the same length. 

        Since adding the end effector lines breaks this same length status,
        ApproximationSpace must use a for loop for some verification, causing 
        the inefficiency.
        ...
        ee_segments = self.batch_create_end_effector_segments(
            end_points[:, -1, :], summed_thetas
        )
