...
class PolygonalRobot(HolonomicRobot):
    ...
    def generate_robot_representation(self, state):
        ...
        robot = create_rectangle_geometry(
            x_loc=x, y_loc=y, x_width=self.robot_width, y_length=self.robot_length
        )
    ...
    def batch_get_robot_representations(self, states: np.ndarray):
        ...
        tmats = np.array([[cosines, -sines, xs], [sines, cosines, ys]])
        ...
        end_points1 = np.concatenate(
            (
                np.zeros((B, 1)),
                np.zeros((B, 1)) + self.robot_length / 2,
                np.ones((B, 1)),
            ),
            axis=1,
        )
        end_points2 = np.concatenate(
            (
                np.zeros((B, 1)),
                np.zeros((B, 1)) - self.robot_length / 2,
                np.ones((B, 1)),
        ...
        end_points = np.concatenate(
            (end_points1.reshape(B, 1, -1), end_points2.reshape(B, 1, -1)), axis=1
        )
    ...
    def batch_sample_points_around_target(self, targets: np.ndarray):
        ...
        thetas = np.random.uniform(
            low=0, high=2 * np.pi, size=(B * num_thetas_per_target, 1)
        )
