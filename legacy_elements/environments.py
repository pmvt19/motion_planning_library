...
from legacy_elements.old_utils import interpolate_euclidean_edge, interpolate_angular_edge, interpolate_edge, interpolate_edge_mobile_arm
...
class PlanarMobileArm(Environment):
    def __init__(self):
        print("Planar Mobile Arm Environment is Bugged and inconsistent with the representations. " \
                "This will be fixed in the environment/robot refactor")
        super().__init__()
        self.base_width = 2
    ...
    def make_state(self, state : np.ndarray):
        return AngularNumpyState(value=state, angular_dims_start=2)
    ...
    def get_2d_rotation_matrix(self, theta):
    ...
    def generate_robot_representation(self, state):
        ...
        for i in range(len(end_effector_point_pairs)):
            ...
            # ee_link = affinity.rotate(ee_link, angle=(theta1+theta2+theta3-np.pi/2), use_radians=True, origin=joint_positions[-1])
    ...
    def is_valid(self, state):
        # self.generate_robot_representation
        self.num_collision_checks += 1
        robot, arms, ee = self.generate_robot_representation(state)
        for obs in self.obstacles:
    ...
    def is_valid_edge(self, start, end):
        if isinstance(start, np.ndarray):
            start = self.make_state(start)
            end = self.make_state(end)
            
        states = [self.make_state(state) for state in interpolate_edge_mobile_arm(start, end, delta=0.1)]
        for state in states:
...
if __name__ == '__main__':
    ...
    env = PlanarMobileArm()
    state = AngularNumpyState(value=np.array([0, 0, np.pi/4, np.pi/2, np.pi/2]), angular_dims_start=2)
    # state = AngularNumpyState(value=np.array([0, 0, 0, 0, 0.0]), angular_dims_start=2)
    # env = DubinsCarEnv()
    ...
    interpolated_edge = interpolate_edge_mobile_arm(state1, state2, delta=0.1)
    print(interpolated_edge)
    plt.show()

    print(env.forward_kinematics(state))
    env.animate_path([env.make_state(state) for state in interpolated_edge], frame_delay=0.01)
